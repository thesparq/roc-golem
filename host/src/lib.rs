...
pub extern "C" fn cabi_realloc(
    _old_ptr: *mut u8, _old_size: usize, align: usize, new_size: usize,
) -> *mut u8 {
...
unsafe fn canon_string_to_roc(buf: *const u8, offset: i32) -> (i32, i32) {
    ...
    let roc_ptr = alloc_roc_string(core::slice::from_raw_parts(
        buf.offset((offset + 4) as isize), len as usize,
    ));
...
unsafe fn write_u8(buf: *mut u8, off: i32, val: u8) -> i32 { core::ptr::write_unaligned(buf.offset(off as isize) as *mut u8, val); off + 1 }
unsafe fn write_i32(buf: *mut u8, off: i32, val: i32) -> i32 { core::ptr::write_unaligned(buf.offset(off as isize) as *mut i32, val); off + 4 }
unsafe fn write_str(buf: *mut u8, off: i32, val: &str) -> i32 {
    let l = val.len() as i32;
    let o = write_i32(buf, off, l);
    if l > 0 { core::ptr::copy_nonoverlapping(val.as_ptr(), buf.offset(o as isize), val.len()); }
    o + l
}
unsafe fn write_empty_list(buf: *mut u8, off: i32) -> i32 { write_i32(buf, off, 0) }
unsafe fn write_variant_disc(buf: *mut u8, off: i32, disc: u8) -> i32 { write_u8(buf, off, disc) }
unsafe fn write_option_none(buf: *mut u8, off: i32) -> i32 { write_u8(buf, off, 0) }
unsafe fn write_enum(buf: *mut u8, off: i32, disc: u8) -> i32 { write_u8(buf, off, disc) }
unsafe fn encode_result_ok_empty(buf: *mut u8, off: i32) -> i32 { write_variant_disc(buf, off, 0) }
unsafe fn encode_result_ok_unit(buf: *mut u8, off: i32) -> i32 { write_variant_disc(buf, off, 0) }
unsafe fn encode_result_ok_data_value_empty(buf: *mut u8, off: i32) -> i32 {
    let o = write_variant_disc(buf, off, 0);
    let o = write_variant_disc(buf, o, 0);
    write_empty_list(buf, o)
}
...
unsafe fn encode_snapshot(buf: *mut u8, off: i32, payload: &[u8], mime: &str) -> i32 {
    let o = write_i32(buf, off, payload.len() as i32);
    if !payload.is_empty() { core::ptr::copy_nonoverlapping(payload.as_ptr(), buf.offset(o as isize), payload.len()); }
    let o = o + payload.len() as i32;
    write_str(buf, o, mime)
}
...
pub extern "C" fn golem_get_definition() -> i32 {
    unsafe {
        ...
        let type_name = if json_str.len() > 10 { extract_json_string_field(json_str, "typeName") } else { "unknown" };
...
unsafe fn extract_json_string_field<'a>(json: &'a str, field: &str) -> &'a str {
    ...
    if field_bytes.is_empty() || json_bytes.len() < field_bytes.len() + 6 { return "unknown"; }
    ...
    for i in 0..(json_len.saturating_sub(field_len + 6)) {
        ...
        for k in 0..field_len { if json_bytes[i + 1 + k] != field_bytes[k] { match_field = false; break; } }
        if !match_field { continue; }
        let colon_start = i + 1 + field_len;
        let mut match_colon = true;
        for k in 0..4 { if json_bytes[colon_start + k] != quote_colon_space[k] { match_colon = false; break; } }
        if !match_colon { continue; }
        let value_start = colon_start + 4;
        if value_start >= json_len { break; }
        for j in value_start..json_len {
            if json_bytes[j] == b'"' { return core::str::from_utf8_unchecked(&json_bytes[value_start..j]); }
        }
        break;
    }
    "unknown"
...
pub extern "C" fn roc_crashed(_msg_ptr: i32, _msg_len: i32) { loop {} }
